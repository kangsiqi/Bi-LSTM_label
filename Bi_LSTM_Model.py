...
class BiLSTM(object):
    ...
    def __init__(
            self, input_embedding_size, sequence_length, hidden_size, output_size, vocab_size, learning_rate):
        ...
        with tf.device('/cpu:0'), tf.name_scope("Embedding"):
            self.W = tf.Variable(tf.random_uniform([vocab_size, input_embedding_size], -0.1, 0.1), trainable=True, name="W",
                            dtype=tf.float32)
            self.embedding_placeholder = tf.placeholder(tf.float32, [vocab_size, input_embedding_size],
        ...
        with tf.name_scope("Model"):
            ...
            # Define forward and backward lstm cell with tensorflow
            ...
            # Linear activation, using rnn inner loop average pooling
